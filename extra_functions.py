...
def polygons2mask(height, width, polygons, image_id,gs):
    num_channels = len(polygons)
    result = np.zeros((num_channels, height, width))
    for mask_channel in range(num_channels):
        result[mask_channel, :, :] = polygons2mask_layer(height, width, polygons[mask_channel], image_id,gs)
    return result


def generate_mask(image_id, height, width, num_mask_channels=10, train = None,gs = None):
    """
    ...
    for mask_channel in range(num_mask_channels):
        ...
        mask[mask_channel, :, :] = polygons2mask_layer(height, width, polygons, image_id,gs)
    return mask
...
def get_shape(image_id,shapes, band=3):
    if band == 3:
...
def read_image_16(image_id,data_path):
